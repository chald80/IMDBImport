...
namespace IMDBImport
{
    class Program
    {
        ...
        {
            SqlConnection sqlConn = new SqlConnection("Server=localhost;Database=IMDB;User Id=sa;Password=Hald20280013!;");
            sqlConn.Open();

            List<TitleBasic> allTitles = ReadAllTitles(@"D:\4.semster\DB\title.basics.tsv\data.tsv", 50000);
        ...
        {
            SqlCommand sqlComm = new SqlCommand("DELETE FROM TitleBasic", sqlConn);
        ...
        public static List<TitleBasic> ReadAllTitles(string filePath, int maxRows)
        {
            ...
            foreach (string line in System.IO.File.ReadLines(@"D:\4.semster\DB\title.basics.tsv\data.tsv"))
            {
